...
"""dhparser_projectServer.py - starts a server (if not already running) for the
                    compilation of dhparser_project
"""
    ...
    try:
        with open(cfg_filename, 'r', encoding='utf-8') as f:
            ...
            if (host, port) != (KNOWN_HOST, KNOWN_PORT):
                verbose('Retrieved host and port _item %s:%i from config file "%s".'
...
class dhparser_projectCPUBoundTasks:
...
class dhparser_projectBlockingTasks:
    def __init__(self, lsp_data: dict):
        from DHParser.lsp import gen_lsp_table
        self.lsp_data = lsp_data
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
...
class dhparser_projectLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "dhparser_project-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = dhparser_projectCPUBoundTasks(self.lsp_data)
        self.blocking = dhparser_projectBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from dhparser_projectParser import batch_process
        args = argstr.split(' ')
    ...
    #     from dhparser_projectParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new dhparser_projectServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('dhparser_project_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    dhparser_project_lsp = dhparser_projectLanguageServerProtocol()
    lsp_table = dhparser_project_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', dhparser_project_lsp.simply_compile)
    dhparser_project_server = Server(rpc_functions=lsp_table,
                        cpu_bound=dhparser_project_lsp.cpu_bound.lsp_table.keys(),
                        blocking=dhparser_project_lsp.blocking.lsp_table.keys(),
                        connection_callback=dhparser_project_lsp.connect,
                        server_name="dhparser_projectServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        dhparser_project_server.echo_log = True if port >= 0 and host else False
        msg = dhparser_project_server.start_logging(log_path.strip('" \''))
        if dhparser_project_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        dhparser_project_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        if overwrite:
            try:
                with open(cfg_filename, 'w', encoding='utf-8') as f:
                    verbose('Storing host and port _item %s:%i in file "%s".'
                          % (host, port, cfg_filename))
        ...
        try:
            ...
            dhparser_project_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing dhparser_project-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python dhparser_projectServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python dhparser_projectServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python dhparser_projectServer.py --stream\n'
             + '    python dhparser_projectServer.py --stopserver\n'
             + '    python dhparser_projectServer.py --status\n'
             + '    python dhparser_projectServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python dhparser_projectServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
